...
export const Main = () => {
  ...
  const filtres=genre_ids.map((genre, index)=>{
      ...
      <div className="contentInfo">
        {
          filtres.map((filter, index)=>{
            return (<span className="genre" key={filter.id}>{filter.name}</span>)
          })}
          ...
          <Grid container spacing={2} gap={4}>
            <Button variant="contained" style={{backgroundColor:"red", fontSize:"1rem", fontWeight:"600"}} endIcon={<PlayCircleFilledWhiteIcon/>}>Stream Now</Button>
            <Button variant="outlined" style={{backgroundColor:"black", color:"white", borderColor:"white", fontSize:"1rem", fontWeight:"600"}} >All Episodes</Button>
          </Grid>
      </div>
      <Grid>
        <SectionCarousel/>
